...
export function AssessmentDetail(){
 ...
 return <Page eyebrow={`Avaliação ${item.number}`} title={`${subjectLabel(item.subject)} — ${item.grade}º ano`} description="Confira o cadastro, configure o gabarito oficial e gere as folhas para impressão.">
  ...
  <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
   ...
   <aside className="space-y-5"><div className="card"><div className="eyebrow">Etapa 3</div><h2 className="text-xl font-bold">Folhas dos alunos</h2><p className="mt-2 text-sm text-black/50">Gere uma folha com QR exclusivo para cada estudante {item.schoolClass?'da turma selecionada':'das turmas compatíveis da rede'}.</p><button className="btn mt-4 flex w-full items-center justify-center gap-2" disabled={!keySaved} onClick={generate}><Users size={18}/> {item.schoolClass?'Gerar para a turma':'Gerar para toda a rede'}</button>{!keySaved&&<p className="mt-2 text-xs text-amber-700">Complete e salve todas as respostas oficiais para liberar a geração.</p>}<div className="mt-4 max-h-80 divide-y overflow-y-auto">{item.sheets?.map(s=><div className="flex items-center justify-between py-3" key={s.id}><span className="text-sm font-semibold">{s.student?.name??'Folha sem aluno'}</span><button aria-label="Baixar PDF do gabarito" className="rounded-lg p-2 text-forest hover:bg-mint" onClick={()=>download(`/api/sheets/${s.id}/pdf`,`gabarito-${s.student?.name??s.id}.pdf`)}><Download size={18}/></button></div>)}</div>{!item.sheets?.length&&<p className="mt-4 text-sm text-black/45">Nenhuma folha gerada.</p>}</div>

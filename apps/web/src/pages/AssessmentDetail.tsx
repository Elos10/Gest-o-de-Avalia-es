...
export function AssessmentDetail(){
 ...
 async function generate(){if(!keySaved){setMessage('Salve o gabarito oficial completo antes de gerar as folhas.');return}try{await api(`/api/assessments/${id}/sheets`,{method:'POST',body:JSON.stringify({allStudents:true})});await load();setMessage('Folhas geradas em pares: dois alunos por página A4. O PDF está pronto para baixar.')}catch(e){setMessage((e as Error).message)}}
 ...
 return <Page eyebrow={`Avaliação ${item.number}`} title={`${subjectLabel(item.subject)} — ${item.grade}º ano`} description="Confira o cadastro, configure o gabarito oficial e gere as folhas para impressão.">
  ...
  <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
   ...
   <aside className="space-y-5"><div className="card"><div className="eyebrow">Etapa 3</div><h2 className="text-xl font-bold">Folhas dos alunos</h2><p className="mt-2 text-sm text-black/50">O PDF agrupa dois alunos por página A4, cada metade com QR exclusivo e pronta para corte.</p><button className="btn mt-4 flex w-full items-center justify-center gap-2" disabled={!keySaved} onClick={generate}><Users size={18}/> {item.schoolClass?'Gerar para a turma':'Gerar para toda a rede'}</button>{!keySaved&&<p className="mt-2 text-xs text-amber-700">Complete e salve todas as respostas oficiais para liberar a geração.</p>}{!!item.sheets?.length&&<><button className="btn mt-3 flex w-full items-center justify-center gap-2" onClick={()=>download(`/api/assessments/${id}/sheets/pdf`,`gabaritos-${item.number}-${item.year}.pdf`)}><Download size={18}/> Baixar PDF da turma</button><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-xs"><thead><tr className="border-b"><th className="pb-2">Número</th><th>Ano</th><th>Unidade</th><th>Série</th><th>Disciplina</th><th>Tempo</th></tr></thead><tbody>{item.sheets.map(s=><tr className="border-b last:border-0" key={s.id}><td className="py-2 font-semibold">{item.number}</td><td>{item.year}</td><td>{item.unit.name}</td><td>{item.grade}º</td><td>{subjectLabel(item.subject)}</td><td>{timeLabels[item.timeMode]}</td></tr>)}</tbody></table></div></>}{!item.sheets?.length&&<p className="mt-4 text-sm text-black/45">Nenhuma folha gerada.</p>}</div>

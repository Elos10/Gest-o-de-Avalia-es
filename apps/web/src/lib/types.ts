...
export type Processing={id:string;status:string;mimeType:string;createdAt:string;errorCode?:string;errorDetail?:string;quality?:{alignment?:number};sheet?:{student?:Student;assessment:Assessment};answers?:Answer[];result?:{id:string;correct:number;wrong:number;blank:number;invalid:number;total:number;percentage:number;score:number};_count?:{answers:number}};

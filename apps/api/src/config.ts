...
export const config=z.object({NODE_ENV:z.enum(['development','test','production']).default('production'),PORT:z.coerce.number().default(3001),WEB_ORIGIN:z.string().url(),SUPABASE_URL:z.string().url(),SUPABASE_PUBLISHABLE_KEY:z.string().min(1),SUPABASE_SECRET_KEY:z.string().min(1).optional(),QR_HMAC_SECRET:z.string().min(32).optional(),CALIBRI_FONT_PATH:z.string().optional(),OMR_PYTHON_PATH:z.string().default('python'),MAX_UPLOAD_BYTES:z.coerce.number().default(15728640),OMR_BLANK_THRESHOLD:z.coerce.number().min(0).max(1).default(.18),OMR_MARKED_THRESHOLD:z.coerce.number().min(0).max(1).default(.42),OMR_DOUBLE_MARK_DELTA:z.coerce.number().min(0).max(1).default(.10),OMR_TRUSTED_CONFIDENCE:z.coerce.number().min(0).max(1).default(.90),OMR_REVIEW_CONFIDENCE:z.coerce.number().min(0).max(1).default(.70)}).parse(process.env);
